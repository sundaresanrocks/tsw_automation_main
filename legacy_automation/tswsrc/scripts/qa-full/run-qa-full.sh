...
# Code Coverage Directory setup check [avimehenwal] currently unused
codecoverage_check () {
    if [ ! -d $WORKSPACE/cc ]   # All
        then  echo "Directory not Found : WORKSPACE/cc"
    elif [ ! -d $DIR_CC ]       # extraction of package is required
        then  echo "Directory not Found : cc/WEB_R008"
    elif [ ! -d $DIR_CC/cobertura ]
        then  echo "Directory not Found : cc/WEB_R008/cobertura"
    elif [ ! -d $DIR_CC/instrumented ]
        then  echo "Directory not Found : cc/WEB_R008/instrumented"
    elif [ ! -d $DIR_CC/tsw ]
        then  echo "Directory not Found : cc/WEB_R008/tsw"
    elif [ ! -d $DIR_CC/crawler ]
        then  echo "Directory not Found : cc/WEB_R008/crawler"
...
${RUN_PY} -t "Build tests - full" ${full_build}
${RUN_PY} -t "Publishing Agent tests - full" ${full_prevalence}
${RUN_PY} -t "Topsites tests - full" ${full_topsites}
...
${RUN_PY} -t "URLDB - full" ${full_urldb}
