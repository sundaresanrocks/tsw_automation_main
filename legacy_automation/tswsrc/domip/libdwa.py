"""
DWA Library Functions
todo
1.variable names change as per convention
"""
import runtime
import logging
        ...
        for domainMappingId in domainMappingIds:
            if domainMappingId == "first_seen" or domainMappingId == "ips_last_changed" or domainMappingId == "ips_last_checked":
                ...
                if (len(dataDict[domainMappingId]) == len(mongoDBDict[domainMappingId])):
                    ...
                    logging.debug("%s length in expected Test case : %s"%(domainMappingId, len(dataDict[domainMappingId])))
            ...
            for dataMappingId in dataMappingIds:
                if dataMappingId == "seen_first" or dataMappingId == "seen_last" or dataMappingId == "active_last_seen":
                    ...
                    if time_diff.days <= 1 and time_diff.seconds <= 22000:
                        logging.info("For IP -> %s of domain %s  : data for %s column is as expected" %(key, domainName, dataMappingId))
                    else:
                        logging.error("For IP -> %s of domain %s  : data for %s column is Not as expected" %(key, domainName, dataMappingId))
                        result = 'fail'
                else:                    
