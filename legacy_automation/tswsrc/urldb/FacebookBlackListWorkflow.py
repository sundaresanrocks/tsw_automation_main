...
from conf.properties import prop_FacebookBlackList_Workflow
...
DEFAULT_SOURCE_DEST = DIR.provider_downloadDirFBB
DEFAULT_ARCFILE_DEST = DIR.provider_archiveDirFBB
DEFAULT_ERROR_DEST= DIR.provider_errorDirFBB
DEFAULT_WORKFILE_DEST=DIR.provider_workingDirFBB

class FaceBookBlackList(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        prop_FacebookBlackList_Workflow()
    ...
    def stage_input_files_to_srcdirectory(self,destfile,srcfile):
        ...
        stdo,stde=workflow_obj.run_security_workflow(workflow_property=runtime.PROP.FaceBookBlackList_workflow)
        return stdo,stde
    ...
    def setUp(self):
        super(FaceBookBlackList, self).setUp()
    ...
    def test_json_creation(self):
        ...
        inputfile= data_path.joinpath('urldb/workflow_fbb/FaceBookBlackListHarvestWorkflow_20161011231823.txt')
        self.stage_input_files_to_srcdirectory('FaceBookBlackListHarvestWorkflow_20161011231823.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        assert len(json_list) == 1
        for json_path in json_list:
            with json_path.open('r') as file:
                file_content = json.load(file)
                assert file_content['source'] == 'FaceBookBlackListHarvestWorkflow'
    ...
    def test_workflow_run(self):
        ...
        """
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/workflow_fbb/FaceBookBlackListHarvestWorkflow_20161011231824.txt')
        stdo=self.stage_input_files_to_srcdirectory('FaceBookBlackListHarvestWorkflow_20161011231824.txt',inputfile)
    ...
    def test_json_queue(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/workflow_fbb/FaceBookBlackListHarvestWorkflow_20161011231825.txt')
        self.stage_input_files_to_srcdirectory('FaceBookBlackListHarvestWorkflow_20161011231825.txt',inputfile)
        sql = "select * from u2.dbo.urldb_queue (nolock) where source='FaceBookBlackListHarvestWorkflow' and pending = 0"
    ...
    def test_specialchar_url(self):
        ...
        sql = "select * from U2.dbo.urldb_queue where source = 'FaceBookBlackListHarvestWorkflow' and pending=1"
