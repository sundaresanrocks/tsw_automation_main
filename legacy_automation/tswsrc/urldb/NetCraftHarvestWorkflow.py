...
from conf.properties import prop_NetCraftHarvest_workflow
...
DEFAULT_SRCFILE_DEST = DIR.provider_sourceDirNetCraft
DEFAULT_WORKFILE_DEST = DIR.provider_workingDirNetCraft
DEFAULT_ARCFILE_DEST = DIR.provider_archiveDirNetCraft
DEFAULT_ERROR_DEST= DIR.provider_errorDirNetCraft

class NetCraftWorkflow(unittest.TestCase):
    ...
    def stage_input_files_to_srcdirectory(self,destfile,srcfile):
        ...
        stdo,stde=workflow_obj.run_security_workflow(workflow_property=runtime.PROP.NetCraftHarvestWorkflow)
        return stdo,stde

    def setUp(self):
        super(NetCraftWorkflow, self).setUp()
    ...
    def test_json_creation(self):
        ...
        inputfile= data_path.joinpath('urldb/NetCraft/872396.txt')
        self.stage_input_files_to_srcdirectory('872396.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        assert len(json_list) == 1
        for json_path in json_list:
            with json_path.open('r') as file:
                file_content = json.load(file)
                assert file_content['source'] == 'NetcraftHarvestWorkflow'

    def test_specialchar_url(self):
        set_prop_application()
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/NetCraft/872410.txt')
        self.stage_input_files_to_srcdirectory('872410.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        urldb_obj.run_urldbqueue_agent()
        sql = "select * from U2.dbo.urldb_queue where source = 'NetcraftHarvestWorkflow' and pending=1"
    ...
    def test_insert_private_ip(self):
        set_prop_application()
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/NetCraft/872414.txt')
        self.stage_input_files_to_srcdirectory('872414.txt',inputfile)
    ...
    def test_shortened_urls(self):
        ...
        set_prop_application()
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/NetCraft/872400.txt')
        stdo1=self.stage_input_files_to_srcdirectory('872400.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        assert len(json_list) == 1
