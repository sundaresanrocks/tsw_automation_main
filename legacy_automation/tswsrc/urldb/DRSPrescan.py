...
from conf.properties import prop_drsprescan_workflow
...
DEFAULT_SRCFILE_DEST = DIR.provider_sourceDirDRSPrescan
DEFAULT_WORKFILE_DEST = DIR.provider_workingDirDRSPrescan
DEFAULT_ARCFILE_DEST = DIR.provider_archiveDirDRSPrescan
DEFAULT_ERROR_DEST= DIR.provider_errorDirDRSPrescan

class DRSPrescanHarvesterWorkflow(unittest.TestCase):
    ...
    def stage_input_files_to_srcdirectory(self,destfile,srcfile):
        ...
        stdo,stde=workflow_obj.run_security_workflow(workflow_property=runtime.PROP.Drsprescan_workflow)
        return stdo,stde

    def setUp(self):
        super(DRSPrescanHarvesterWorkflow, self).setUp()
    ...
    def test_json_creation(self):
        ...
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134410.txt')
        self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134410.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        assert len(json_list) == 1
        for json_path in json_list:
            with json_path.open('r') as file:
                file_content = json.load(file)
                assert file_content['source'] == 'DRSprescanHarvestWorkflow'
    ...
    def test_workflow_run(self):
        ...
        """
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134411.txt')
        stdo=self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134411.txt',inputfile)
    ...
    def test_json_queue(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134412.txt')
        self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134412.txt',inputfile)
        sql = "select * from u2.dbo.urldb_queue (nolock) where source='DRSprescanHarvestWorkflow' and pending = 0"
    ...
    def test_movetoworkingdir(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134412.txt')
        stdo,stde=self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134444.txt',inputfile)
        string='/data/webcache/workflows/harvesters/DRSprescanHarvestWorkflow/working/DRSprescanHarvestWorkflow_20161020134444.txt'
        assert self.check_string_stdout(string,str(stdo))== True

    def test_checkerrordirectory(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134413.txt')
        self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134413.txt',inputfile)
    ...
    def test_specialchar_url(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_20161020134419.txt')
        self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_20161020134419.txt',inputfile)
        json_list = self.urldb_cobj.list_urldb_dir()
        urldb_obj.run_urldbqueue_agent()
        sql = "select * from U2.dbo.urldb_queue where source = 'DRSprescanHarvestWorkflow' and pending=1"
    ...
    def test_insert_private_ip(self):
        self.give_cleanslate()
        inputfile= data_path.joinpath('urldb/DRSPrescan/DRSprescanHarvestWorkflow_2016102013420.txt')
        self.stage_input_files_to_srcdirectory('DRSprescanHarvestWorkflow_2016102013420.txt',inputfile)
