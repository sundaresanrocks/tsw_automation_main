...
__author__ = 'https://github.com/imbolc/mongo'

from ca.ca_config import MongoDBSettings
...
class Cursor(pymongo.cursor.Cursor):
    ...
    def __next__(self):
        data = pymongo.cursor.Cursor.__next__(self)
...
class Collection(pymongo.collection.Collection):
    ...
    def find_one(self, *args, **kwargs):
        data = pymongo.collection.Collection.find_one(self, *args, **kwargs)
        if data:
...
class DocumentBase(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        def db(cls):
            ...
            new_db = cls.conn[cls.__database__]
            if hasattr(cls, '__auth__'):
                assert new_db.authenticate(*cls.__auth__), 'Failed authenticate'
            return new_db
        new_class.db = db
        ...
        def coll(cls):
            ...
            new_coll = Collection(cls.db, coll_name, document_class=cls)
            for index in getattr(cls, '__indexes__', []):
                index.ensure(new_coll)
...
class Document(dict, metaclass=DocumentBase):
    def __init__(self, *args, **kwargs):
...
class CompetitorMongoDoc(Document):
    __connection__ = {'host': MongoDBSettings.host, 'port': MongoDBSettings.port}
    __database__ = MongoDBSettings.dbname
    __safe__ = True
    __collection__ = MongoDBSettings.collection_name